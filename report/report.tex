...
DepChain was built in two incremental stages. The first stage focused on implementing a consensus layer based on the Byzantine Read/Write Epoch Consensus algorithm. To support this, we implemented a layered communication stack over UDP, including abstractions such as FairLossLinks, StubbornLinks (with authenticated acknowledgments to prevent denial-of-service attacks), ConditionalCollect, and AuthenticatedPerfectLinks. Our consensus engine supports deciding a sequence of values in a single instance, possible by bundling transactions in blocks.
...
In the remainder of this report, we describe the architecture, security considerations, and implementation choices behind DepChain.
...
All messages between clients and blockchain members, as well as between the blockchain members themselves, are serialized using Protocol Buffers. Protocol Buffers were used due to their performance and the fact that they are not vulnerable to remote code execution expoits associated with serializing and deserializing objects. Communication is performed using UDP sockets, with custom abstractions built on top to provide reliability and security. These abstractions include:

\begin{itemize}
    \item \textbf{FairLossLinks (FLL)}: A basic unreliable communication abstraction that mimics the behavior of UDP.
    \item \textbf{StubbornLinks (SL)}: Ensures eventual delivery by repeatedly sending messages until an acknowledgment is received. To prevent denial-of-service attacks, acknowledgments are sent using the authenticated perfect links.
...
\begin{itemize}
    \item A Besu's \texttt{SimpleWorld} instance to simulate the world state.
...
At the core of DepChain lies a Byzantine consensus protocol based on the Read/Write Epoch Consensus model studied in class. Our implementation follows the version described in Algorithms 5.17 and 5.18 of the course book.
